...
//const ClientProfile = ({ name, entries }) => {
  ...
  Problem = () => {
    // Implement canceling logic here
    alert('User is not Registered! Please logout and make a new account.');
  }
  onSubmitSignIn = (event) => {
   ...
   // console.log(id);
    ...
    if (Object.values(errors).every(error => error === "")) { // Check if all errors are empty
      ...
      fetch('http://localhost:3000/clientprofile',{
        method: 'post',
    ...
    <div>
      ...
      <div className="container mt-4 center" >
        <form id="clientForm ">
          ...
          <select id="state" name="state" required value={this.state.selectedState} onChange={(e) => this.setState({ selectedState: e.target.value })}>
